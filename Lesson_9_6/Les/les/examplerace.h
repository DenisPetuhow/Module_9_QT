...
public:
    ...
    ~Controller(){
        ...
        workerThread.quit(); // убирает поток из цикла событий
        workerThread.wait(); // блокирует поток, пока он не завершит работу

    }

signals:
    void operate( uint32_t *num, bool mutexOn, int numIterat ); //по вызову сигнала запускается работа, которая обрабат в потоке
